...
parser.add_argument('-c', '--chromosome', dest="chromosome", default='',help="Enter the path to the file  which contain the data downloaded automatically from Ensembl (Homo_sapiens.GRCh38.104.chromosome.1.dat) for the funcitonnal boxes and only the number of chromosome for the Regulatory")
...
if Type=='BoitesReg':
    cmdReg='python3 {}/BoitesRegulatrices/boitesRegulatrices2.py.py -c {} -e {} -r {}'.format(selfDirback,chromosome,espece,pathResult+"/json")
    for i in listPlage:
        cmdplage+=' -p ' + " ".join(map(str,i))
...
if Type=='Protein':
    cmd3='python3 {}/UniprotInteractomics/request3.py  -c {} -j {} -t {} -r {}'.format(selfDirback,chromosome,json,pathTableInteractants,pathResult+"/UniprotInteractomics")
    cmd3+=cmdplage
    os.system(cmd3)
